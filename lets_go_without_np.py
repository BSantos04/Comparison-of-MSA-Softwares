...
def t_test(values):
    ...
    p_values = []
    for value in values:
        t_stat, p_value = stats.ttest_1samp([value], mean_value)
        p_values.append(p_value)

    # Determine the "best" value based on the lowest p-value (indicating it's significantly different)
    min_value = min(p_values)
    best_value_index = p_values.index(min_value)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        all_memories = {"MAFFT": [], "MUSCLE": [], "T-Coffee": [], "ClustalOmega": []}
        all_times = {"MAFFT": [], "MUSCLE": [], "T-Coffee": [], "ClustalOmega": []}
        all_cpus = {"MAFFT": [], "MUSCLE": [], "T-Coffee": [], "ClustalOmega": []}
        all_sp_scores = {"MAFFT": [], "MUSCLE": [], "T-Coffee": [], "ClustalOmega": []}
        ...
        for i in range(5):
            ...
            all_sp_scores["MAFFT"].append(spscore.sp_score(mafft_info[0]))
    
            all_memories["MUSCLE"].append(muscle_info[1])
            all_times["MUSCLE"].append(muscle_info[2])
            all_cpus["MUSCLE"].append(muscle_info[3])
            all_sp_scores["MUSCLE"].append(spscore.sp_score(muscle_info[0]))
            ...
            all_sp_scores["T-Coffee"].append(spscore.sp_score(tcoffee_info[0]))
            ...
            all_sp_scores["ClustalOmega"].append(spscore.sp_score(clustalo_info[0]))
        ...
        o_scores = {}
        for tool in all_memories.keys():
            normalized_sp_score = overall_score(best_sp_scores, best_sp_scores[tool], 5)
